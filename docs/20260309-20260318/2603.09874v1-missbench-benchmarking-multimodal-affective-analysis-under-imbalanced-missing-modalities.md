...
tags: ["query:mm-fus"]
score: 8.0
evidence: 不平衡模态缺失下的多模态分析基准测试
tldr: 针对多模态情感计算中模态缺失率不均衡的现实问题，本文提出了MissBench基准测试框架。该框架在四个主流数据集上标准化了共享及不均衡缺失协议，并引入模态公平性指数（MEI）和模态学习指数（MLI）来诊断模型表现。研究发现，现有模型在不均衡缺失下存在显著的模态不公平和优化失衡，为构建更鲁棒的情感分析模型提供了重要工具。
selection_source: fresh_fetch
motivation: 传统评估通常假设各模态缺失率相同，忽略了现实应用中不同模态因成本或脆弱性导致的缺失率不均衡及训练偏差问题。
method: 提出了MissBench基准框架，并定义了MEI和MLI两个诊断指标，分别用于衡量不同模态贡献的公平性以及训练过程中的优化平衡度。
result: 实验证明，在共享缺失率下表现稳健的模型，在不均衡缺失条件下往往会暴露严重的模态不公平和梯度优化失衡问题。
conclusion: MissBench及其诊断指标为在现实不完整模态场景下进行多模态情感模型的压力测试和深入分析提供了实用方案。
---
