...
result: 实验发现顶尖模型在处理基础任务时表现尚可，但在使用专业工具时因空间定位精度不足导致性能下降。
conclusion: 该研究通过连接静态图像感知与交互式临床工作流，为构建可审计、处理全研究数据的医学影像智能体提供了重要框架。
---

## 摘要
目前，在医学影像任务中评估视觉语言模型（VLM）时，由于依赖于需要大量人工筛选的预选2D图像，往往过度简化了临床现实。这种设置忽略了现实世界诊断的核心挑战：一个真正的临床智能体必须能够主动在跨多个序列或模态的完整3D体数据中导航，以收集证据并最终支持最终决策。为了解决这一问题，我们提出了 MEDOPENCLAW，这是一个可审计的运行时环境，旨在让 VLM 在标准医学工具或查看器（如 3D Slicer）中动态运行。在此运行时之上，我们引入了 MEDFLOWBENCH，这是一个涵盖多序列脑部 MRI 和肺部 CT/PET 的全研究医学影像基准。它系统地评估了医学智能体在仅查看器、工具使用和开放方法赛道上的能力。初步结果揭示了一个关键见解：虽然最先进的 LLM/VLM（例如 Gemini 3.1 Pro 和 GPT-5.4）可以成功操作查看器以解决基础的研究级任务，但由于缺乏精确的空间定位，当赋予其使用专业辅助工具的权限时，其性能反而会下降。通过弥合静态图像感知与交互式临床工作流之间的差距，MEDOPENCLAW 和 MEDFLOWBENCH 为开发可审计的全研究医学影像智能体奠定了可重复的基础。
