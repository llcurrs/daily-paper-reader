...
tags: ["query:mm-fus"]
score: 7.0
evidence: 合成同步 RGB-LiDAR 实例的多模态增强
tldr: 针对自动驾驶数据集中长尾分布导致的稀疏类别感知难题，本文提出VERIA框架。该框架采用图像优先的多模态增强策略，利用基础模型合成同步的RGB-LiDAR实例，并通过严格的语义和几何验证机制筛选高质量样本。VERIA有效提升了稀疏类别的类内多样性，在nuScenes和Lyft数据集上显著增强了3D目标检测性能。
selection_source: fresh_fetch
motivation: 现有的长尾3D检测增强方法在细粒度多样性和场景上下文一致性方面存在局限，难以覆盖稀疏类别的变化空间。
method: 提出一种以验证为中心的多模态增强框架，利用基础模型合成实例，并结合顺序语义与几何验证来确保生成样本的真实性。
result: 在nuScenes和Lyft数据集上，该方法显著提升了纯LiDAR和多模态设置下稀疏类别的3D目标检测精度。
conclusion: VERIA通过结合生成式模型与多阶段验证，为解决3D感知中的长尾分布问题提供了一种高效且具有诊断性的增强方案。
---

## 摘要
驾驶数据集中的长尾分布对 3D 感知提出了根本性挑战，因为稀有类别表现出显著的类内多样性，但现有样本仅稀疏地覆盖了这一变化空间。现有的基于复制粘贴或资产库的实例增强方法虽然提高了稀有类别的曝光率，但在细粒度多样性和场景上下文放置方面往往受到限制。我们提出了 VERIA，这是一种图像优先的多模态增强框架，它利用现成的基础模型合成同步的 RGB-LiDAR 实例，并通过顺序的语义和几何验证对其进行筛选。这种以验证为中心的设计倾向于选择更符合真实 LiDAR 统计数据且涵盖更广泛类内变化的实例。分阶段的产量分解（Stage-wise yield decomposition）为流水线的可靠性提供了基于日志的诊断。在 nuScenes 和 Lyft 数据集上，VERIA 在仅 LiDAR 和多模态设置下均提升了稀有类别的 3D 目标检测性能。我们的代码可在 https://sgvr.kaist.ac.kr/VERIA/ 获取。
