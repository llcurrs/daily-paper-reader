...
result: 通过对训练模型、确定性模型、集成模型及随机生成模型的对比评估，验证了生成式方法在处理室内地图补全中的有效性。
conclusion: FlatLands为不确定性感知的室内制图和具身导航提供了重要的数据支持和评估基准。
---

## 摘要
单张第一人称图像通常仅能捕捉到地板的一小部分，然而，周围环境的完整度量可通行性地图能更好地服务于室内导航等应用。我们介绍了 FlatLands，这是一个用于单视角鸟瞰图（BEV）平面图补全的数据集和基准。该数据集包含来自 6 个现有数据集的 17,656 个真实度量室内场景的 270,575 次观测，并配有对齐的观测图、可见性图、有效性图和地面真值 BEV 地图；该基准包括分布内和分布外评估协议。我们对比了无需训练的方法、确定性模型、集成模型以及随机生成模型。最后，我们将该任务实例化为一个端到端的单目 RGB 到平面图的流水线。FlatLands 为具身导航中的不确定性感知室内建图和生成式补全提供了一个严谨的测试平台。
