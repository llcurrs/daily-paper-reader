...
tags: ["query:mm-fus"]
score: 8.0
evidence: 多模态融合中用于模态分布映射的修正流
tldr: 针对多模态融合中的模态间隙问题，本文提出CaReFlow框架。该方法利用修正流将一种模态分布映射到另一种，通过“一对多映射”策略让源模态观测目标模态的全局分布，缓解配对数据不足。结合自适应松弛对齐和循环修正流机制，在保证分布对齐的同时防止信息丢失。实验证明，该方法在多模态情感计算任务上表现优异，有效缩小了模态间隙。
selection_source: fresh_fetch
motivation: 传统的模态对齐方法多关注一对一映射，缺乏对目标模态全局分布的感知，且受限于配对数据的不足。
method: 提出基于修正流的分布映射方法，引入一对多映射策略、自适应松弛对齐机制以及循环一致性约束。
result: 在多个多模态情感计算任务上取得了极具竞争力的结果，可视化结果证实其能有效减小模态间隙。
conclusion: CaReFlow通过改进的修正流技术实现了更稳健的模态分布对齐，为多模态融合提供了一种高效的新方案。
---

## 摘要
模态间隙显著限制了多模态融合的效果。以往的方法通常采用扩散模型和对抗学习等技术来缩小模态间隙，但它们通常侧重于一对一的对齐，而没有让源模态的数据点接触到目标模态的全局分布信息。为此，利用修正流（rectified flow）能够通过直线轨迹将一个分布映射到另一个分布的特性，我们将修正流扩展到模态分布映射中。具体而言，我们利用了修正流中的“一对多映射”策略，使源模态的每个数据点都能观察到整体的目标分布。这也缓解了每个样本内配对数据不足的问题，从而实现了更稳健的分布转换。此外，为了实现更准确的分布映射并解决一对多映射中流向模糊的问题，我们设计了“自适应松弛对齐”，对属于同一样本的模态对实施更严格的对齐，而对不属于同一样本或类别的模态对应用松弛映射。此外，为了防止分布映射过程中的信息丢失，我们引入了“循环修正流”，以确保转换后的特征可以被翻译回原始特征，从而使多模态表示能够学习到充足的模态特有信息。在分布对齐之后，即使采用简单的融合方法，我们的方法在多模态情感计算的多个任务上也取得了非常有竞争力的结果，可视化结果验证了该方法能有效缩小模态间隙。
